"""Fitting and read-only evaluation for the medical protocol (fork of final runner).
...
import time
import numpy as np
from .numeric import fit_temperature, probabilities, metrics, choose_lora, degrade, replay_difference
...
def _gpu_ready():
    ...
    if os.environ.get('CUBLAS_WORKSPACE_CONFIG') != ':4096:8':
        raise RuntimeError('Launch through run_aperture_medical.sh for deterministic CUDA configuration')
    ...
    with store.lock(),_loaded(job) as (model,backend,_):
        ...
        try:
            if job['arm'].startswith('lora'):
                ...
                bases,spectra=extract_basis(model,sites,fit,backend.loss_and_mask,rank=job['rank'],mode=job['basis_mode'],seed=job['basis_seed'])
...
def execute_evaluate(job, selection_path):
    ...
    with store.lock(),_loaded(job,source) as (model,backend,controller):
        calibration_samples=_samples(job,'calibration')
        restored=[backend.score(s,controller) for s in calibration_samples]
        saved=read_jsonl(source/'calibration_predictions.jsonl')
        if [r['sample_id'] for r in saved]!=[r['sample_id'] for r in restored]:
            raise ValueError('restored calibration ID mismatch')
        replay=replay_difference([r['mean_log_scores'] for r in saved],
                                 [r['mean_log_scores'] for r in restored])
        atomic_json(folder/'restore_replay.json',replay)
        if not replay['passed']:raise RuntimeError('saved-state replay failed before query evaluation')
        ...
        artifacts=['clean.jsonl','restore_replay.json']
        try:
