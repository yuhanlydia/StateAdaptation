"""Multicenter fitting and read-only evaluation.

Derived from aperture_final/worker.py; historical entry points are untouched.
Changes: model family, matched random controller, strict deterministic settings,
patient-disjoint validation, and mandatory support-only repeat-fit gate.
...
from .protocol import check_job, source_signature, ARMS
from aperture_final.protocol import probe_ids, write_locked
...
def seal_choices(jobs, destination):
    ...
    for key,arms in units.items():
        if set(arms)!=set(next(iter(arms.values()))['required_arms']):raise ValueError(f'incomplete planned arms for {key}')
        data={};seals={}
...
def _loaded(job, restore=None):
    ...
    try:
        configure_determinism(job)
        seed_everything(job['optimization_seed'])
        lora=job['arm'].startswith('lora')
        model,processor=load_task_model(job['model_path'],job['family'],use_lora=lora,
        ...
        if restore is not None:
            ...
            elif job['arm'] in ('aperture','random'):
    ...
    with store.lock(),_loaded(job) as (model,backend,_):
        ...
        try:
            if job['arm'].startswith('lora'):
                ...
                bases,spectra=extract_basis(model,sites,fit,backend.loss_and_mask,rank=job['rank'],mode=('random' if job['arm']=='random' else 'covariance'),seed=job['basis_seed'])
                ...
                stats.update(optimizer_updates=len(history),optimized_scalars=controller.num_scalars(),
                             ...
                             fitting_visits=len(fit)*job['steps'],basis_visits=(0 if job['arm']=='random' else len(fit)),
                             operator_norms={k:float(torch.linalg.matrix_norm(a.detach().cpu(),2)) for k,a in controller.operators().items()})
            elif job['arm']=='frozen':
...
def configure_determinism(job):
    """Fail on unsupported deterministic kernels; never relax to get a win."""
    import os
    import torch
    if job.get('strict_determinism',True):
...
def check_repeatability(first,second,destination,tolerance=.005):
    """Compare a repeated fit on held-out SUPPORT scores, never query outcomes."""
    first=Path(first);second=Path(second)
    verified_record(first);verified_record(second)
    a=read_jsonl(first/'calibration_predictions.jsonl');b=read_jsonl(second/'calibration_predictions.jsonl')
    if [(x['sample_id'],x['label_id']) for x in a]!=[(x['sample_id'],x['label_id']) for x in b]:
        raise ValueError('repeat calibration identities differ')
    ...
    if x.shape!=y.shape or not x.size or not np.isfinite(x).all() or not np.isfinite(y).all():
        raise ValueError('invalid repeat scores')
    delta=float(np.max(np.abs(x-y)))
    r={'passed':delta<=tolerance,'max_abs_candidate_score_difference':delta,
       'tolerance':tolerance,'source_signature':source_signature(),'query_forwards':0,
       'first':str(first.resolve()),'second':str(second.resolve()),
       'first_seal':file_hash(first/'DONE.json'),'second_seal':file_hash(second/'DONE.json'),
       'note':'Support-only repeated fit. Do not replace a canonical model with the repeat.'}
    write_locked(destination,r)
    if not r['passed']:raise RuntimeError('support-only repeatability gate failed; diagnose before querying, do not increase tolerance')
    return r


def require_repeatability(job):
    p=Path(job['repeat_gate']);r=json.loads(p.read_text())
    if not r.get('passed') or r['source_signature']!=source_signature():raise ValueError('repeatability gate missing, failed, or stale')
    for name in ('first','second'):
        verified_record(r[name])
        if file_hash(Path(r[name])/'DONE.json')!=r[name+'_seal']:raise ValueError('repeatability source modified')
